...
class DCfg extends CApplicationComponent
{
    ...
    public function deleteDbItems()
    {
        ...
        if($this->getUseCache())
            DBase::getApp()->cache->delete($this->getCacheId());
    }
    ...
    {        
	   ...
	   if($this->getCreateTable())
        ...
        if($this->getUseCache())
            $items=DBase::getApp()->cache->get($this->getCacheId(), self::CACHE_GROUP);
        ...
        if(!$items)
        {
		  ...
		  $connection = DBase::getApp()->getDb();
            ...
            if($this->getUseCache())
                DBase::getApp()->cache->save($items,$this->getCacheId(), self::CACHE_GROUP); 
        }
        $this->setItem($items);
    }
    ...
    public function loadConfigFile($fileArray,$keyName=null)
    {
        ...
        {
            if(file_exists($file=DBase::getPath('application.config').DIRECTORY_SEPARATOR.trim($fileArray).'.php'))
            {
    ...
    {
        return DBase::app()->getDb()->tablePrefix ? DBase::app()->getDb()->tablePrefix . $this->_tableName : $this->_tableName;
    }
     ...
     */
    public function handleRequestEnds()
    {
	   if(count($this->_saveItemsToDatabase)>0)
        ...
        if($this->_cacheNeedsFlush && $this->getUseCache())
        	DBase::app()->cache->delete($this->getCacheId(), self::CACHE_GROUP);
    } 
