...
class DDebugPanel
{
	...
	public function render($items = array(), $config = array())
	{
		...
		include(Yii::app()->findLocalizedFile($viewFile,'ta_in'));
	}
}
...
class DDebugClass
{
	...
	{		
		...
		foreach ($items as $item)
		{
			...
			$time=DDebugTrace::timestampToTime($timestamp);
			$odd = !$odd;
...
class DDebugDB extends DDebugClass
{
	public static $_data;
	...
	/*public static function getInfo($data, $config = null)
	{
		...
		if (count($items) > 0) $result['content'] = DDebugTrace::render($items);
	...
	public static function getInfo($data=null, $config = null)
	{
		if(!self::$_data)
			$data = array();
		else
		...
		$result = array();
		$result['panelTitle'] = 'Profiling Summary Report';
		$count = count($data);
		
		$result['title'] = 'Profiling Summary Report: '.$count;
		if ($count > 0) $result['content'] = self::render($data);
		return $result;
	}
	...
	public static function render($items)
	{
		...
		foreach ($items as $item)
		{
			list($proc, $count, $min, $max, $total) = $item;
			$odd = !$odd;
			$result .='<tr'.($odd?' class="odd"':'').'><td>'.$proc.'</td><td>'.$count.'</td><td>'.sprintf('%0.5f',$total).'</td><td>'.sprintf('%0.5f',$total/$count).'</td><td>'.sprintf('%0.5f',$min).'</td><td>'.sprintf('%0.5f',$max).'</td></tr>';
		}
		
		if($result !== '') 
			$result = '<tbody>'.$result.'</tbody>';
		
		$result = '<table><thead><tr><th>Procedure</th><th>Count</th><th>Total (s)</th>	<th>Avg. (s)</th><th>Min. (s)</th>	<th>Max. (s)</th></tr></thead>'.$result.'</table>';
		
		return $result;
	}
}

class DDebugTrace extends DDebugClass
{
	public static function getInfo($data, $config = null)
	{
		parent::getInfo($data);
...
class DDebugTime extends DDebugClass
{
	public static function getInfo($data, $config = null)
	{
		parent::getInfo($data);
...
class DDebugMem extends DDebugClass
{
	public static function getInfo($data, $config = null)
	{
		parent::getInfo($data);
...
class DDebugConfig extends DDebugClass
{
	...
	public static function frameworkInfoAsArray()
	{
		$values = array(
			...
			'version'   => DBase::getVersion('system')
		);		
		return $values;
	}
	
	public static function appInfoAsArray()
	{
		$values = array(
			'name'  => 'dotPHP Development Team',
			'version'   => DBase::getVersion('application')
		);		
		return $values;
	}
	...
	public static function formatArrayAsHtml($id, $values, $highlight = false)
	{
	...
	public static function getInfo($data, $config = null)
	{
		...
		$result = array();
		$result['title'] = 'System Log';
		$result['headinfo'] = self::getHeadInfo();
		$result['panelTitle'] = 'Configuration';
		$result['content']  = self::formatArrayAsHtml('framework', self::frameworkInfoAsArray(),true);
		$result['content'] .= self::formatArrayAsHtml('application', self::appInfoAsArray(),true);
		...
		$result['content'] .= self::formatArrayAsHtml('Framework::Application', self::YiiAppAsArray() , true);
 ...
 */
class DDebug extends CLogRoute
{
	public $config = '';
	public $allowedIPs = array('127.0.0.1');
	public $profilerEnabled = false;
	...
	public function processLogs($logs)
	{
		...
		$items[] = DDebugConfig::getInfo($logs, $config);
		$items[] = DDebugMem::getInfo($logs);
		$items[] = DDebugTime::getInfo($logs);
		$items[] = DDebugDB::getInfo();
		$items[] = DDebugTrace::getInfo($logs);

		$panel = new DDebugPanel();
